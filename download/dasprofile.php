...
$adminRow = mysqli_fetch_assoc($query);


if (!$adminRow) {
  die("<h4 style='color:red; text-align:center;'> No admin record found for username: $username</h4>");
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $update = "UPDATE admin SET AdminName='$adminName', ContactNumber='$contact' ,Photo='$img' WHERE Username='$username'";
  $res = mysqli_query($con, $update);

  if ($res) {
    echo "<script>alert('Profile updated successfully!');</script>";
    echo "<script>window.location.href='dasprofile.php';</script>";
  } else {
...
<body>
    ...
    <div class="d-flex align-items-center">
      ...
      <div class="dropdown me-3 text-center">
        ...
        <ul class="dropdown-menu dropdown-menu-end mt-2 p-2 shadow-sm" aria-labelledby="notificationBell">
          ...
          <li style='color:blue;text-size:1px'>
            <?php
            $sql = "SELECT * FROM  appoinment";
            $res = mysqli_query($con, $sql);
            $num = mysqli_num_rows($res);
          ...
          <li class="text-center">
            <a href="new-Appointment.php"
              class="see-all text-primary text-decoration-none small">See all
              notifications</a>
          </li>
        </ul>
      </div>
  ...
  <div class="page-wrapper d-flex" style="min-height:100vh;">
    ...
    <div class="main-content flex-grow-1 p-3" id="main-content">
      <div class="container-fluid mt-3" id="cards-container">
        <div class="row g-3">
          <div class="col-lg-12">


            <div class="container mt-5">
              <h1 class="text-center text-danger">Admin Profile</h1>
                ...
                <div class="card-body">
                  <form method="POST" enctype="multipart/form-data">
                    ...
                    <input type="text" class="form-control" name="adminname"
                      value="<?php echo htmlspecialchars($adminRow['AdminName']); ?>" required>

                    <label class="form-label mt-3">User Name</label>
                    <input type="text" class="form-control" name="username"
                      value="<?php echo htmlspecialchars($adminRow['Username']); ?>" readonly>

                    <label class="form-label mt-3">Contact Number</label>
                    <input type="text" class="form-control" name="contactnumber"
                      value="<?php echo htmlspecialchars($adminRow['ContactNumber']); ?>" required>

                    <label class="form-label mt-3">Email Address</label>
                    <input type="email" class="form-control" name="email"
                      value="<?php echo htmlspecialchars($adminRow['Email']); ?>" readonly>


                    <label class="form-label mt-3">Image</label>
                    <input type="file" class="form-control" name="image">

                    <button type="submit" class="btn btn-info text-white w-100 mt-4">Update</button>
                  </form>
                </div>
              </div>
            </div>
          ...
          <hr>
          <div class="col-sm-12">
            <p class="text-center">BPMS Admin Panel.</p>
          </div>
        </div>
      </div>
