...
<style>
  ...
  .page-wrapper {
    ...
    width: 100%;
    overflow-x: hidden;
    /* poore page pe horizontal scroll disable */
  }
</style>
<?php
session_start();
    ...
    <div class="d-flex align-items-center">
      ...
      <div class="dropdown me-3 text-center">
        ...
        echo "<div class='dv'><span class='data'>$num</span></div>"
          ?>
        <i class="fa fa-bell text-dark fs-5" id="notificationBell" data-bs-toggle="dropdown" style="cursor:pointer;">
  ...
  <div class="page-wrapper d-flex" style="min-height:100vh;">
    ...
    <div class="sidebar bg-light p-3" id="sidebar" style="width:250px;">
      <ul class="nav flex-column px-2 mt-5 gap-4">
        ...
        <li class="nav-item"> <a href="#" class="nav-link d-flex justify-content-between align-items-center"> <span><i
                class="fa fa-briefcase me-2"></i>Services</span> <i class="fa fa-angle-left"></i> </a>
          <ul class="submenu">
        ...
        <li class="nav-item"> <a href="#" class="nav-link d-flex justify-content-between align-items-center"> <span><i
                class="fa fa-calendar-check me-2"></i>Enquiry</span> <i class="fa fa-angle-left"></i> </a>
          <ul class="submenu">
            <li><a href="read-Enquiry.php">Read Enquiry</a></li>
            <li><a href="unread-Enquiry.php">Unread Enquiry</a></li>
          </ul>
        </li>
        <li class="nav-item"> <a href="customer-List.php" class="nav-link"><i class="fa fa-users me-2"></i>Customer
            List</a> </li>
          ...
          <div class="col-lg-12 mt-5 bg-dark text-light">
            <div class="table-responsive">
              <table class="table border table-hover table-dark">
                <u>
                  <h1 class="text-center fw-bold ">All Appointment Details</h1>
                </u>
                <tr>
                ...
                <input type='hidden' name='AppNumber' value='{$row['AppNumber']}'>
                <button type='submit' name='delete' class='btn btn-danger btn-sm'>Reject</button>
                 <button type='submit' name='accept' class='btn btn-danger btn-sm'>Accept</button>
                </form>
               </td>
               </tr>";
                ...
                if (isset($_POST['delete'])) {
                  ...
                  $select = "SELECT * FROM appoinment WHERE AppNumber='$AppNumber'";
                  $resSelect = mysqli_query($con, $select);
                  $row = mysqli_fetch_assoc($resSelect);

                  if ($row) {

                    $insert = "INSERT INTO reject_appointment (AppNumber, Name, Mo_Number, AppDate, AppTime, User_img, Status)
                   VALUES ('{$row['AppNumber']}', '{$row['Name']}', '{$row['Mo_Number']}', '{$row['AppDate']}',
                           '{$row['AppTime']}', '{$row['User_img']}', 'Rejected')";
                    $resInsert = mysqli_query($con, $insert);

                    if ($resInsert) {

                      $delete = "DELETE FROM appoinment WHERE AppNumber='$AppNumber'";
                      mysqli_query($con, $delete);
                      echo "<script>alert('Appointment rejected successfully!'); window.location.href='all-Appointment.php';</script>";
                    }
                  }
                }
                ...
                if (isset($_POST['accept'])) {
                  ...
                  if ($row) {
                    // Move to accept_appointment
                    $insert = "INSERT INTO accept_appointment (AppNumber, Name, Mo_Number, AppDate, AppTime, User_img, Status)
                   VALUES ('{$row['AppNumber']}', '{$row['Name']}', '{$row['Mo_Number']}', '{$row['AppDate']}',
                           '{$row['AppTime']}', '{$row['User_img']}', 'Accepted')";
                    $resInsert = mysqli_query($con, $insert);
                    ...
                    if ($resInsert) {
                      ...
                      $delete = "DELETE FROM appoinment WHERE AppNumber='$AppNumber'";
                      mysqli_query($con, $delete);
                      echo "<script>alert('Appointment accepted successfully!'); window.location.href='all-Appointment.php';</script>";
                    }
                  }
                }
                ?>
